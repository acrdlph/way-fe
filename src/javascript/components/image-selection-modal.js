...
const isValidFileName = filename => filename.endsWith('.jpg') || filename.endsWith('.png');
      ...
      reader.onload = function (e) {
        ...
        while (offset < length) {
          ...
          if (marker == 0xffe1) {
            ...
            offset += 2;
            for (let i = 0; i < tags; i++) if (view.getUint16(offset + i * 12, little) == 0x0112) return resolve(view.getUint16(offset + i * 12 + 8, little));
  ...
  render() {
    ...
    const okButton = (
      <div className="image-selection-button image-selection-button-ok">
        <MuiThemeProvider>
          <RaisedButton onClick={this.uploadImage} backgroundColor="#68a0ce" label="OK" />
        </MuiThemeProvider>
      </div>
    );
    ...
    const rotateButton = (
      <div className="image-selection-button">
        <MuiThemeProvider>
          <RaisedButton onClick={this.rotate} backgroundColor="#68a0ce" label="Rotate" />
        </MuiThemeProvider>
      </div>
    );
    ...
    const oldPicVisibleClass = this.state.hasSelectedFile || !this.props.user.data.photo
      ? 'image-selection-pic-invisible'
      : '';

    return (
      <div className="image-selection">
        <Modal isOpen contentLabel="Modal" style={style}>
          <div>
            ...
            <div className={`image-selection-preview ${previewVisibleClass}`}>
              ...
              <canvas
                width="100"
                height="100"
                id="img-preview-canvas"
                className="image-selection-canvas"
              />
            </div>
            ...
            <div className="image-selection-button image-selection-button-cancel">
              <MuiThemeProvider>
                <RaisedButton onClick={this.props.close} backgroundColor="#68a0ce" label="Cancel" />
              </MuiThemeProvider>
            </div>
          </div>
        </Modal>
